...
class File extends Api{
    ...
    public function upload_record(){
        load()->func('file');
        $temp_arr = explode(".", $_FILES['upload_record']['name']);
        $file_ext = array_pop($temp_arr);
        $file_ext = trim($file_ext);
        $file_ext = strtolower($file_ext);
        ...
        $img_name = time().'.'.$file_ext;
        //保存上传文件
        file_move($_FILES['upload_record']['tmp_name'], MODULE_ROOT . '/lib/web/upload_record/'.$img_name);
        $this->result(0,'上传成功',MODULE_URL.'lib/web/upload_record/'.$img_name);
    }
}