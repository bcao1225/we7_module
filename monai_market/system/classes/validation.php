...
class Validation
{
    ...
    public function getMenu($full = false)
    {
        ...
        if (!empty($allmenus)) {
            ...
            foreach ($allmenus as $key => $val) {
                if(in_array($key,['user','set','information'])){
                    if ($this->cv($key)) {
    ...
    public function headarr()
    {
        global $_GPC,$_W;
        $uniacid = $_W['uniacid'];
        $count = pdo_fetchcolumn("select count(*) from ".tablename('monai_market_car_detail')." where `uniacid`='$uniacid' and `audit_type`=0");
        $result=pdo_fetch("SELECT * FROM ".tablename('monai_market_info')." where `uniacid`='$uniacid' order by id desc limit 1");
        $header_arr = array('count'=>$count,'name'=>$result['name'],'logo'=>$result['logo']);
        return $header_arr;
    }
