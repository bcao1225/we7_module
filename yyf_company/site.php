...
//                ->setCellValue('B1', '图片');
//
//            $kami = pdo_getall('z_poster');
//            foreach ($kami as $k => $v) {
//                $k +=2;
//
//                // 图片生成
//                $objDrawing[$k] = new \PHPExcel_Worksheet_Drawing();
//                $objDrawing[$k]->setPath('../attachment/hb/'.$v['src']);
//                // 设置宽度高度
//                $objDrawing[$k]->setHeight(600);//照片高度
//                $objDrawing[$k]->setWidth(400); //照片宽度
//                /*设置图片要插入的单元格*/
//                $objDrawing[$k]->setCoordinates('C'.$k);
//                // 图片偏移距离
//                $objDrawing[$k]->setOffsetX(410);
//                $objDrawing[$k]->setOffsetY(610);
//                $objDrawing[$k]->setWorksheet($objPHPExcel->getActiveSheet());
//            }
//            $objPHPExcel->getActiveSheet()->setTitle('批量导出');
//            $objPHPExcel->setActiveSheetIndex(0);
//
...
//            exit;
	...
	public function doWebXianxiadakuan()
	{
		...
		if(!empty($_POST['name'])){
			$where = $_POST['name'];
			$list=pdo_fetchall("select *,a.id as aid,a.carttime as acarttime,a.status as astatus from `ims_z_oredercard` as a left join `ims_z_user` as b on a.user_id=b.id where b.`name` like '%{$where}%' ".$status."  order by a.id desc");
		}else{
			$uniacid=$_W['uniacid'];
			$participators = pdo_fetchall("SELECT `id` FROM " . tablename('z_oredercard') . " where 1 ".$status1." ORDER BY `id` DESC");
			$total = count($participators);
			if(!isset($_GPC['page'])){$pageindex=1;}else{$pageindex = intval($_GPC['page']);}
			$pagesize =15;
			$pager = pagination($total, $pageindex, $pagesize);
			$p = ($pageindex-1) * 15;
			$list=pdo_fetchall("select *,a.id as aid,a.carttime as acarttime,a.status as astatus from `ims_z_oredercard` as a left join `ims_z_user` as b on a.user_id=b.id where 1 ".$status." order by a.id desc limit " . $p . "," . $pagesize);
		}
		foreach ($list as $key => $value) {
		    $list[$key]['acarttime']=date('Y-m-d H:m:s',$list[$key]['acarttime']);
		}
