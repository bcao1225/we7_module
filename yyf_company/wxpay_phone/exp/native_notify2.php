...
class NativeNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		//$aaa = $data["transaction_id"];
		
		$aaa = "000";
	

		$r=$dosql->getone("select id from #@__goodsorder where orderlistnum='{$id}' and checkinfo=0 "); 
		if(is_array($r)){

			$sql="UPDATE `#@__goodsorder` SET checkinfo='1', paynum='$aaa' WHERE  orderlistnum='{$id}' and checkinfo=0";
			// 商品出售排行在此处修改
			if($dosql->ExecNoneQuery($sql))
			{
				...
				if($cfg_delhouse==1)//用户设为待发货减库存
				{
					...
					WHERE `gorderlistnum`='{$id}' AND `Status`='order'";
