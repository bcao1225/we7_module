...
switch ($_GPC['action']) {
    ...
    case 'batch':
        for ($i = $_GPC['min']; $i <= $_GPC['max']; $i++) {
            pdo_insert('ims_book_store_bookrack', ['id' => $i, 'guild_id' => $_GPC['guild_id']]);
    ...
    case 'add':
        if ($_W['ispost']) {
            pdo_insert('ims_book_store_bookrack', ['id' => $_GPC['id'], 'guild_id' => $_GPC['guild_id']]);
    ...
    case 'delete':
        /*先删除当前书架对应的书*/
        pdo_delete('ims_book_store_book', ['guild_id' => $_GPC['guild_id'], 'bookrack_id' => $_GPC['id']]);
        /*再删除书架*/
    ...
    default:
        ...
        foreach ($bookrack_list as $index => $bookrack) {
            $books = pdo_getall('ims_book_store_book', ['guild_id' => $_GPC['guild_id'], 'bookrack_id' => $bookrack['id']]);
